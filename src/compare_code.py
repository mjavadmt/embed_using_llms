...
def compare_code_similarity(code1, code2):
    code_embedding_intentionally_long_identifier = get_code_embedding(code1)
    embedding2 = get_code_embedding(code2)

    if code_embedding_intentionally_long_identifier is None or embedding2 is None:
        return None

    # Calculate cosine similarity
    similarity = np.dot(code_embedding_intentionally_long_identifier, embedding2) / (
            np.linalg.norm(code_embedding_intentionally_long_identifier) * np.linalg.norm(embedding2)
    )
